...
public class UtilidadesDireccion {
    private static final Map<String, Provincia> provincias = new HashMap<>();
    static {
    ...
    public static boolean esCPValido(String cp,String nombreProvincia) {
        if (cp.matches("[0-5][0-9]{4}")) {
            String codigo = cp.substring(0, 2);
            Provincia provincia = provincias.get(nombreProvincia);
            return provincias.values().stream().anyMatch(p -> p.getCodigo().equals(codigo));
        }
        return false;
    }
